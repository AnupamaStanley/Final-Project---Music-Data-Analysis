...
object DataEnrichment {
  def main(args: Array[String]): Unit = {
    ...
    val load_data = s"""INSERT OVERWRITE TABLE enriched_data PARTITION (batchid, status) SELECT i.user_id,
                        ...
                        LEFT OUTER JOIN song_artist_map sa ON i.song_id = sa.song_id  WHERE i.batchid=$batchId  """
    try {
      sparkSession.sqlContext.sql("SET hive.auto.convert.join=false")
      sparkSession.sqlContext.sql("SET hive.exec.dynamic.partition.mode=nonstrict")
      sparkSession.sqlContext.sql("USE project")

      sparkSession.sqlContext.sql(create_hive_table)
      sparkSession.sqlContext.sql(load_data)
    }
    catch{
